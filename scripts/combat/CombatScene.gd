...
var _pieces:   Dictionary = {}   # name -> Node3D
var _grid_pos: Dictionary = {}   # name -> Vector2i
var _occupied: Dictionary = {}   # Vector2i -> String (name)
...
# Mark all occupied cells as solid except 'from' and 'to'
...
return []
raw_path.remove_at(0)  # drop starting cell
...
# Move: try to reach a cell adjacent to the hero
...
# Attack if now adjacent to hero
...
_log("[color=tomato][b]%s frappe %s : %d degats![/b][/color]" \
% [cname, target.name, result.get("damage", 0)])
if h_piece != null:
await _animate_flash(h_piece as Node3D, Color(1.0, 0.15, 0.15))
_update_hp_label(target.name)
else:
_log("%s attaque %s et rate (d20=%d)." \
% [cname, target.name, result.get("roll", 0)])
else:
...
func _do_move(cname: String, from: Vector2i, to: Vector2i) -> Array[Vector2i]:
# Compute path BEFORE updating _occupied so 'to' is not marked solid
var path := _get_path(from, to, 99)
...
_log("[color=yellow]%s touche %s : %d degats! (d20=%d)[/color]" \
% [hero.name, monster.name, result.get("damage", 0), result.get("roll", 0)])
...
# Update occupied AFTER computing path
...
if piece != null:
_animate_move(piece as Node3D, path)  # fire-and-forget for hero turn
